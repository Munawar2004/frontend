...
const Navbar = () => {
  ...
  const [defaultAddressId, setDefaultAddressId] = useState(localStorage.getItem("defaultAddressId") || null);
  ...
  const fetchAddresses = async () => {
    ...
    try {
      ...
      if (defaultId && !data.data.some(addr => addr._id === defaultId)) {
        localStorage.removeItem("defaultAddressId");
        setDefaultAddressId(null);
      }
    } catch (err) {
      ...
      <nav className="navbar">
        ...
        {user ? (
          <div className="user-dropdown" ref={dropdownRef}>
            <button className="user-button" onClick={() => setDropdownVisible(!dropdownVisible)}>
              {user.name} ▼
            </button>
            {dropdownVisible && (
              <div className="dropdown-menu">
                <button onClick={() => navigate("/profile")}>👤 Profile</button>
                <button onClick={openAddressForm}>🏠 Address</button>
              ...
              <div className="saved-address-list">
                ...
                <ul>
                  {savedAddresses.map((addr) => (
                    <li key={addr._id} className="saved-address-item">
                      <div>
                        {addr.id === defaultAddressId && <span className="default-badge">⭐ DEFAULT</span>}
                        📍 {addr.area}, {addr.city}, {addr.landmark}, Floor: {addr.floor} ({addr.type})
                      </div>
